...
// @desc    Get suppliers summary with dues
// @access  Private
router.get('/summary', protect, canAccessModule('inventory'), async (req, res) => {
    try {
        const summary = await Supplier.aggregate([
            { $match: { isActive: true } },
            {
                $group: {
                    _id: '$type',
                    count: { $sum: 1 },
                    totalPurchase: { $sum: '$totalPurchaseAmount' },
        ...
        const grandTotal = await Supplier.aggregate([
            { $match: { isActive: true } },
            {
                $group: {
                    _id: null,
        ...
        res.json({
            byType: summary,
            total: grandTotal[0] || { totalSuppliers: 0, totalPurchase: 0, totalPayment: 0, totalDues: 0 }
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error', error: error.message });
