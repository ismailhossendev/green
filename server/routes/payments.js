...
router.post('/supplier', protect, canAccessModule('purchase'), async (req, res) => {
    try {
        ...
        // Update supplier
        supplier.totalPayment += amount;
        supplier.totalDues -= amount;
        await supplier.save();
...
router.post('/purchase', protect, canAccessModule('purchase'), async (req, res) => {
    try {
        ...
        const payment = await Payment.create({
            ...
            referenceName: purchase.supplier.name,
            referenceNo: purchase.purchaseNo,
            amount,
            paymentMethod,
            description: description || `Payment for ${purchase.purchaseNo}`,
            addedBy: req.user._id
        });

        // Update purchase
        purchase.paidAmount += amount;
        purchase.dues -= amount;
        await purchase.save();
...
// @desc    Record dealer/customer payment (receive)
// @access  Private
router.post('/dealer', protect, canAccessModule('sales'), async (req, res) => {
    try {
        ...
        const payment = await Payment.create({
            type: customer.type === 'Dealer' ? 'Dealer' : 'Customer',
...
// @route   POST /api/payments/employee
// @desc    Record employee payment (salary)
...
router.post('/employee', protect, authorize('Admin'), async (req, res) => {
    try {
        ...
        const payment = await Payment.create({
            ...
            amount,
            paymentMethod,
            description: description || `Salary payment to ${employee.name}`,
            addedBy: req.user._id
        });
...
// @route   POST /api/payments/others
// @desc    Record other payments
// @access  Private
