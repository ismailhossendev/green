...
// @route   GET /api/hrm/employees/summary
// @desc    Get employee summary by status and department
// @access  Private (Admin, Manager)
router.get('/employees/summary', protect, authorize('Admin', 'Manager'), async (req, res) => {
    try {
        const byStatus = await User.aggregate([
            { $match: { role: { $in: ['Staff', 'Sales', 'Manager'] } } },
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 },
        ...
        const byDepartment = await User.aggregate([
            { $match: { role: { $in: ['Staff', 'Sales', 'Manager'] }, status: 'Active' } },
            {
                $group: {
                    _id: '$department',
                    count: { $sum: 1 },
...
// @route   PUT /api/hrm/employees/:id/status
// @desc    Update employee status (Active/Terminated/Resigned)
// @access  Private (Admin)
...
// @route   POST /api/hrm/attendance
// @desc    Mark attendance (with GPS and photo)
// @access  Private
...
// @route   GET /api/hrm/targets
// @desc    Get sales targets
// @access  Private
...
router.post('/targets', protect, authorize('Admin', 'Manager'), async (req, res) => {
    try {
        ...
        if (target) {
            ...
            target.collectionTarget = collectionTarget;
            await target.save();
        } else {
            // Create new
...
// @route   GET /api/hrm/targets/achievement
// @desc    Get target vs achievement
// @access  Private
router.get('/targets/achievement', protect, canAccessModule('hrm'), async (req, res) => {
    try {
        ...
        const achievements = await Promise.all(employees.map(async (emp) => {
            // Get target
            ...
            const salesData = sales[0] || { totalAmount: 0, totalQty: 0, totalCollection: 0 };
            ...
            return {
                ...
                target: {
                    amount: target?.targetAmount || 0,
                    qty: target?.targetQty || 0,
                    collection: target?.collectionTarget || 0
                },
                achieved: {
                    amount: salesData.totalAmount,
                    qty: salesData.totalQty,
                    collection: salesData.totalCollection
                },
                percentage: {
                    amount: target?.targetAmount ? ((salesData.totalAmount / target.targetAmount) * 100).toFixed(2) : 0,
                    collection: target?.collectionTarget ? ((salesData.totalCollection / target.collectionTarget) * 100).toFixed(2) : 0
                }
            };
        }));
...
router.get('/payroll', protect, authorize('Admin'), async (req, res) => {
    try {
        ...
        const employees = await User.find({
            role: { $in: ['Staff', 'Sales', 'Manager'] },
            status: 'Active'
        }).select('name designation salary taDA loanBalance');
        ...
        const payrollData = await Promise.all(employees.map(async (emp) => {
            ...
            return {
                ...
                salary: emp.salary,
                taDA: emp.taDA,
                loanDeduction: Math.min(emp.loanBalance, emp.salary * 0.1), // Max 10% deduction
                attendance: {
                    present: attendance,
                    workingDays: new Date(currentYear, currentMonth, 0).getDate()
                },
                sales: salesData,
                netPayable: emp.salary + emp.taDA - Math.min(emp.loanBalance, emp.salary * 0.1)
            };
        }));
...
// @route   POST /api/hrm/payroll/pay
...
router.post('/payroll/pay', protect, authorize('Admin'), async (req, res) => {
    try {
        ...
        const payment = await Payment.create({
            ...
            amount,
            description: description || `Salary payment for ${employee.name}`,
            addedBy: req.user._id
        });

        // Deduct loan if applicable
        if (employee.loanBalance > 0) {
            const loanDeduction = Math.min(employee.loanBalance, amount * 0.1);
            employee.loanBalance -= loanDeduction;
            await employee.save();
        }
