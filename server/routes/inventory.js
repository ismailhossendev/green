...
router.get('/', protect, canAccessModule('inventory'), async (req, res) => {
    try {
        ...
        // Search Filter (Regex for partial matching)
        if (search) {
            query.$or = [
        ...
        if (stockStatus) {
            if (stockStatus === 'in-stock') {
                query['$expr'] = {
                    $gt: [{ $add: ["$stock.goodQty", "$stock.badQty", "$stock.damageQty", "$stock.repairQty"] }, 0]
                };
            } else if (stockStatus === 'out-of-stock') {
                query['$expr'] = {
                    $eq: [{ $add: ["$stock.goodQty", "$stock.badQty", "$stock.damageQty", "$stock.repairQty"] }, 0]
                };
            } else if (stockStatus === 'low-stock') {
                query['$expr'] = {
                    $lt: [{ $add: ["$stock.goodQty", "$stock.badQty", "$stock.damageQty", "$stock.repairQty"] }, 5]
                };
            }
        }
...
// @desc    Get stock summary by brand and type
// @access  Private
router.get('/summary', protect, canAccessModule('inventory'), async (req, res) => {
    try {
        const summary = await Product.aggregate([
            ...
            {
                $group: {
                    _id: { brand: '$brand', type: '$type' },
                    totalGood: { $sum: '$stock.goodQty' },
                    totalBad: { $sum: '$stock.badQty' },
                    totalDamage: { $sum: '$stock.damageQty' },
                    totalRepair: { $sum: '$stock.repairQty' },
...
// @desc    Transfer stock between conditions (Good -> Bad, Bad -> Repair, etc.)
// @access  Private (Admin, Manager, Staff)
router.post('/:id/transfer', protect, authorize('Admin', 'Manager', 'Staff'), async (req, res) => {
    try {
        ...
        if (!validConditions.includes(from) || !validConditions.includes(to)) {
        ...
        if (product.stock[from] < qty) {
            return res.status(400).json({ message: `Insufficient ${from} stock` });
        }
...
// @desc    Check product active usage (Replacements, Stock)
// @access  Private
router.get('/:id/usage', protect, canAccessModule('inventory'), async (req, res) => {
    try {
        ...
        // 1. Check Active Replacements (Pending, Checked, Sent to Factory)
        const activeReplacementsCount = await Replacement.countDocuments({
            'items.product': req.params.id,
            status: { $in: ['Pending', 'Checked', 'Sent to Factory'] }
        });

        // 2. Check Stock
        const stockTotal = (product.stock?.goodQty || 0) +
            (product.stock?.badQty || 0) +
            (product.stock?.damageQty || 0) +
            (product.stock?.repairQty || 0);

        res.json({
            activeReplacements: activeReplacementsCount,
