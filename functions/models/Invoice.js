...
const InvoiceSchema = new mongoose.Schema({
    ...
    priceType: { type: String, enum: ['Retail', 'Dealer'], default: 'Retail' },
...
InvoiceSchema.pre('validate', async function (next) {
    if (!this.invoiceNo) {
        ...
        const lastInvoice = await this.constructor.findOne({ brand: this.brand }).sort({ createdAt: -1 }).select('invoiceNo');
        let nextNum = 1;
        if (lastInvoice && lastInvoice.invoiceNo) { const match = lastInvoice.invoiceNo.match(/(\d+)$/); if (match) { nextNum = parseInt(match[1]) + 1; } }
