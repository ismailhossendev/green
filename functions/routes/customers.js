...
router.get('/', protect, canAccessModule('customers'), async (req, res) => {
    try {
        ...
        const customers = await Customer.find(query).skip((page - 1) * limit).limit(parseInt(limit)).sort({ name: 1 });
...
router.get('/summary', protect, canAccessModule('customers'), async (req, res) => {
    try {
        ...
        const summary = await Customer.aggregate([{ $match: match }, { $group: { _id: null, totalCustomers: { $sum: 1 }, totalSalesQty: { $sum: '$totalSalesQty' }, totalSalesAmount: { $sum: '$totalSalesAmount' }, totalPayment: { $sum: '$totalPayment' }, totalAdjust: { $sum: '$totalAdjust' }, totalDues: { $sum: '$totalDues' } } }]);
        res.json(summary[0] || { totalCustomers: 0, totalSalesQty: 0, totalSalesAmount: 0, totalPayment: 0, totalAdjust: 0, totalDues: 0 });
    } catch (error) { console.error(error); res.status(500).json({ message: 'Server error', error: error.message }); }
});

router.get('/district-summary', protect, canAccessModule('customers'), async (req, res) => {
    try {
        const summary = await Customer.aggregate([{ $match: { isActive: true } }, { $group: { _id: '$district', customerCount: { $sum: 1 }, totalSalesQty: { $sum: '$totalSalesQty' }, totalSalesAmount: { $sum: '$totalSalesAmount' }, totalPayment: { $sum: '$totalPayment' }, totalDues: { $sum: '$totalDues' } } }, { $sort: { totalSalesAmount: -1 } }]);
        res.json(summary);
    } catch (error) { console.error(error); res.status(500).json({ message: 'Server error', error: error.message }); }
});

router.get('/districts', protect, canAccessModule('customers'), async (req, res) => {
    try { const districts = await Customer.distinct('district'); res.json(districts.filter(d => d)); }
    catch (error) { console.error(error); res.status(500).json({ message: 'Server error', error: error.message }); }
});
...
router.post('/', protect, canAccessModule('customers'), async (req, res) => {
    try {
        ...
        const customer = await Customer.create({ name, phone, email, address, district, brand: brand || 'Both', type: type || 'Retail' });
        if (req.body.openingDues && parseFloat(req.body.openingDues) > 0) {
            const amount = parseFloat(req.body.openingDues);
            customer.totalDues = amount; customer.totalAdjust = amount;
            await customer.save();
            const Ledger = require('../models/Ledger');
            await Ledger.create({ customer: customer._id, brand: customer.brand, type: 'Opening', referenceId: customer._id, debit: amount, credit: 0, balance: amount, description: 'Opening Balance', addedBy: req.user._id });
        }
